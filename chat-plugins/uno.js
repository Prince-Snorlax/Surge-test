...
class UNOgame extends Rooms.RoomGame {
	...
	leaveGame(user) {
		if (this.state === 'signups' && this.removePlayer(user)) {
			this.sendToRoom(`|html|${Server.nameColor(user.name, true, true)} has left the game of UNO.`);
			return true;
		}
		return false;
	}
	...
	getPlayers(showCards) {
		...
		return playerList.map(id => `${(this.currentPlayer && this.currentPlayer === id ? "<strong>" : "")}${Server.nameColor(this.players[id].name, false, true)} (${this.players[id].hand.length}) ${(this.currentPlayer && this.currentPlayer === id ? "</strong>" : "")}`);
	}
		...
		this.onAwaitUno()
			.then(() => {
				...
				this.timer = setTimeout(() => {
					this.sendToRoom(`${Server.nameColor(player.name, true, true)} has been automatically disqualified.`);
	...
	onRunEffect(value, initialize) {
		...
		case 'Wild':
			...
			this.state = 'color';
			this.timer = setTimeout(() => {
				this.sendToRoom(`${Server.nameColor(this.players[this.currentPlayer].name, true, true)} has been automatically disqualified.`);
				this.eliminate(this.currentPlayer);
			}, this.maxTime * 1000);
			break;
		}
	...
	onWin(player) {
		...
		for (let i = 0; i < this.players.length; i++) {
			Server.addExp(Users(this.players[i]).userid, this.room, 20);
		}
...
exports.commands = {
	uno: {
		...
		pass: function (target, room, user) {
			...
			room.game.sendToRoom(`|html|${Server.nameColor(user.name, true, true)} has passed.`);
			room.game.nextTurn();
		},
