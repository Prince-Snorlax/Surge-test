...
var colors = {'red': 4, 'blue': 5, 'yellow': 4, 'green': 4, 'black': 8};
...
var Roulette = (function () {
    ...
    Roulette.prototype.placeBet = function (user, color, self) {
        if (!this.players[user.userid]) {
           ...
           Db('money').set(user.userid, Db('money').get(user.userid) - 1);
        } else {
    ...
    Roulette.prototype.deleteBets = function (user, self) {
        ...
        Db('money', user.userid, this.players[user.userid].bets, '+');
    ...
    Roulette.prototype.end = function (user) {
        for (var i in this.players)
            Db('money', i, this.players[i].bets, '+');
    ...
    Roulette.prototype.spin = function () {
        ...
        if (winners.length <= 0) {
            ...
            winners.forEach(function (user) {
                user = Users.get(user);
               Db('money').set(toId(user.userid), Db('money').get(toId(user), 0) + winamount).get(toId(user.userid));
            });
        }
        clearTimeout(this.timer);
        delete roulettes[this.room.id];
...
var cmds = {
    ...
    participants: function (target, room, user) {
        ...
        this.sendReplyBox('Number of Roulette players: ' + Object.keys(roulettes[room.id].players).length);
    },
    ...
    bet: function (target, room, user) {
        if (!roulettes[room.id]) return this.sendReply('There is no roulette going on in this room right now.');
        if (Db('money').get(user.userid, 0) < 1) return this.errorReply("You don't have enough bucks to join this game.");
        target = toId(target);
